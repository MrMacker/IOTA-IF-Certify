  ...
  create() {
    return this.finalize(async (state) => {
      ...
      try {
        ...
        validated = state.request.predicates.every((predicate) =>
          credentials.some(predicate)
        );

        console.log("Presentation", validated ? "valid" : "invalid");
      } catch (error) {
        console.log("Presentation invalid because:", error.message);
      }
