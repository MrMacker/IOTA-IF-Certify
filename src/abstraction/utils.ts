...
import { Duration, Timestamp } from "@iota/identity-wasm/node";

export { Duration } from "@iota/identity-wasm/node";
...
export function delay(duration: Duration) {
  const currentTime = Date.parse(Timestamp.nowUTC().toRFC3339());
  const targetTime = Date.parse(
    Timestamp.nowUTC().checkedAdd(duration).toRFC3339()
  );
